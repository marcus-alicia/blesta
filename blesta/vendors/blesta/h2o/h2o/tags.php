...
class For_Tag extends H2o_Node
{
    ...
    function __construct($argstring, $parser, $position)
    {
        ...
        if (!$this->item) {
            [$this->key, $this->item] = [$this->item, $this->key];
        }
...
class Block_Tag extends H2o_Node
{
    ...
    function __construct($argstring, $parser, $position)
    {
        ...
        $this->stack = [$parser->parse('endblock', "endblock {$this->name}")];
...
class With_Tag extends H2o_Node
{
    ...
    function render($context, $stream)
    {
        ...
        $context->push([$this->shortcut => $variable]);
        $this->nodelist->render($context, $stream);
        $context->pop();
    }
}
...
H2o::addTag(['block', 'extends', 'include', 'if', 'ifchanged', 'for', 'with', 'cycle', 'load', 'debug', 'comment', 'now', 'autoescape', 'csrf_token']);
