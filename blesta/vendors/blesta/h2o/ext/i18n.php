...
h2o::addLookup(['H2o_I18n', 'gettext']);
...
class H2o_I18n
{
    ...
    function setupGettext()
    {
        ...
        if (!exec($this->gettext_path . "xgettext -V")) {
            throw new Exception("xgettext binary cannot be found, if you are using Windows system either install through cygwin
    ...
    function extract()
    {
        ...
        # get all tempaltes
        $templates = $this->getTemplates($this->searchpath, $this->extensions, [dirname(dirname($this->gettext_path))]);
    ...
    function getPluralForm($locale = 'en')
    {
        ...
        $map = [
            ...
            'ja' => ["1", "0"],
            'zh' => ["1", "0"],
            'pl' => ["3", "(n==1 ? 0 : n%10>=2 && n%10< =4 && (n%100<10 or n%100>=20) ? 1 : 2)"],
            'ro' => ["3", "(n==1 ? 0 : (n==0 or (n%100 > 0 && n%100 < 20)) ? 1 : 2)"],
            'ru' => ["3", "(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10< =4 && (n%100<10 or n%100>=20) ? 1 : 2)"],
        ];
    ...
    function getTemplates($path, $exts, $exclude = [])
    {
        $results = [];
        foreach (new DirectoryIterator($path) as $f) {
            ...
            } else if ($f->isDir() && !in_array($f->getPathname(), $exclude)) {
...
function templize($source)
{
    ...
    while ($t = $tokenstream->next()) {
        if ($in_block) {
            ...
            } else if ($t->type == 'block' && $t->content == 'plural') {
                $is_plural = true;
            } else if ($t->type == 'text') {
                if ($is_plural) {
            ...
            if ($t->type == 'block') {
                ...
                } else if (preg_match($block_re, $t->content, $matches)) {
                    $in_block = true;
                }
            } else if ($t->type == 'variable') {
