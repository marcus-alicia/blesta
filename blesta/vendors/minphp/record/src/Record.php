 ...
 */
class Record extends PdoConnection
{
     ...
     * @param boolean $bind_value True to treat $value as a bound value
     *  (i.e. a string or integer, rather than an table or table field)
     ...
     *
     * @return mixed The return value on success depends on the fetch type.
     ...
     *
     * @return mixed An array containing all of the remaining rows in the result set, false on failure.
    ...
    private function buildQuery()
    {
        ...
        switch ($this->type) {
            case "delete":
                $sql = "DELETE " . $this->buildColumns() . " FROM "
                    . $this->buildTables() . $this->buildWhere()
                    . $this->buildLimit();
                break;
            case "insert":
                $sql = "INSERT INTO " . $this->buildTables()
                    . $this->buildValues() . $this->buildOnDuplicate();
                break;
            case "update":
                $sql = "UPDATE " . $this->buildTables() . " SET "
                    . $this->buildValuePairs($this->fields)
                    . $this->buildWhere() . $this->buildLimit();
                break;
            case "select":
                $sql = "SELECT " . $this->buildColumns() . " FROM "
                    . $this->buildTables() . $this->buildWhere()
                    . $this->buildGroup() . $this->buildHaving()
                    . $this->buildOrder() . $this->buildLimit();
                break;
            case "create_if_not_exists":
            case "create":
                $sql = "CREATE TABLE "
                    . (
                        $this->type == "create_if_not_exists"
                        ? "IF NOT EXISTS "
                        : ""
                    ) . $this->buildTables() . $this->buildFields()
                    . $this->buildTableOptions();
                break;
            case "alter":
                $sql = "ALTER TABLE " . $this->buildTables()
        ...
        foreach ($this->fields as $name => $field) {
            ...
            if ($action == "add") {
                ...
                $field_str = $this->escapeField($name) . " " . $field[$action]['type'] .
                    (isset($field[$action]['size'])
                        ? "(" . $field[$action]['size'] . ")"
                        : "") .
                    (isset($field[$action]['unsigned']) && $field[$action]['unsigned']
                        ? " UNSIGNED "
                        : "") .
                    (isset($field[$action]['is_null']) && $field[$action]['is_null']
                        ? " NULL"
                        : " NOT NULL") .
                    (isset($field[$action]['default'])
                        ? " DEFAULT ?"
                        : "") .
                    (isset($field[$action]['auto_increment']) && $field[$action]['auto_increment']
                        ? " AUTO_INCREMENT"
                        : "");
            }
                ...
                foreach ($key as $name => $field) {
                    ...
                    foreach ($field[$action] as $field_name) {
                        $field_str .= (
                            ...
                            . $this->escapeField($field_name);
                    }

                    $key_field = strtoupper($type) . (
                            $type != "index"
                            ? " KEY "
                            : " "
                        )
        ...
        if (is_array($this->columns)) {
            for ($i = 0, $j = 0; $i < count($this->columns); $i++) {
                ...
                foreach ($this->columns[$i]['fields'] as $key => $value) {
                    ...
                    if (!is_numeric($key)) {
                        $sql .= (
                            $this->columns[$i]['escape']
                                ? $this->escapeField($key)
                                : $key
                            )
                            . " AS " . $this->escapeField($value);
                    } else {
                        $sql .= $this->columns[$i]['escape']
                            ? $this->escapeField($value)
                            : $value;
                    }
                }
    ...
    private function buildOnDuplicate()
    {
        ...
        if (!empty($this->duplicate)) {
            $sql .= " ON DUPLICATE KEY UPDATE "
                . $this->buildConditionals($this->duplicate, false);
        }
        return $sql;
    }
        ...
        if (!empty($this->order)) {
            ...
            for ($i = 0; $i < count($this->order); $i++) {
                $sql .= (
                    ...
                    (
                        $this->order[$i]['escape']
                        ? $this->quoteIdentifier($this->order[$i]['field'])
                        : $this->order[$i]['field']
                    ) .
                    (
        ...
        if (!empty($this->fields)) {
            ...
            foreach ($this->fields as $field => $value) {
                ...
                if (is_array($value)) {
                    if (isset($value['bind_value']) && $value['bind_value'] === false) {
                        $values .= (
                            isset($value['escape']) && $value['escape']
                            ? $this->escapeField($value['value'])
                            : $value['value']
                        );
                    } else {
                        $values .= "?";
    ...
    private function buildValuePairs($pairs)
    {
        ...
        if (!empty($pairs)) {
            ...
            foreach ($pairs as $key => $value) {
                ...
                if (is_array($value)) {
                    if (isset($value['bind_value']) && $value['bind_value'] === false) {
                        $sql .= isset($value['escape']) && $value['escape']
                            ? $this->escapeField($value['value'])
                            : $value['value'];
                    } else {
                        $sql .= "?";
